...
describe('CircuitBreakerStrategy', () => {
	...
	it('should short-circuit', async () => {
		...
		await expect(strategy.process(observable).toPromise()).resolves.toEqual('fallback');
	...
	it('should reset after sleepWindowInMilliseconds', async () => {
		...
		await expect(strategy.process(of(1)).toPromise()).resolves.toEqual(1);
	...
	it('should not short-circuit', async () => {
		...
		await expect(strategy.process(observable).toPromise()).resolves.toEqual(1);
		expect(strategy['isClosed']).toBeTruthy();
		await expect(strategy.process(observable).toPromise()).resolves.toEqual(1);
	});
});
